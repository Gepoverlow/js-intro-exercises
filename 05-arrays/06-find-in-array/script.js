...
(function () {
  ...
  function findJean(array) {
    let search = array.find(
      (person) => person.firstname === "Jean" && person.lastname === "Dupont"
    );
    if (search) {
      console.log(
        `Jeans email is ${
          search.email
        } and his index in the array is ${findJeanIndex(
          array,
          "Jean",
          "Dupont"
        )}`
      ...
      console.log("Jean was not found :(");
    }
  }

  function findJeanIndex(array, name, surname) {
    return array.findIndex(
      (person) => person.firstname === name && person.lastname === surname
    );
  }
